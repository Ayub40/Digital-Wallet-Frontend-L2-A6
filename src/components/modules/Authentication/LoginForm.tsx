...
export function LoginForm({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (data) => {
        try {
            ...
            const typedErr = err as FetchBaseQueryError & { data?: { message?: string; errors?: any } };

            console.error(typedErr);

            // Field specific errors
            if (typedErr.data?.errors) {
                Object.keys(typedErr.data.errors).forEach((key) => {
                    toast.error(`${typedErr.data?.errors[key]}`);
                });
            }

            // Generic backend message
            if (typedErr.data?.message) {
                toast.error(typedErr.data.message);
